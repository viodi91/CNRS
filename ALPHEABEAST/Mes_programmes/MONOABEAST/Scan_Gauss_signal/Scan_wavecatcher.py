...
def append_to_csv(csv_filename, position, delta, threshold, elapsed_time,
                  c0_hits, c0_rate, file_name, status='SUCCESS'):
...
def perform_measurement_at_position(base_file_name, position, csv_filename, max_position):
    ...
    all_images = [
        wavecatcher_window, green_button, confirmation_window, save_window, config_button,
    ...
    dac_value = FIXED_THRESHOLD

    # fixe le threshold à 244
    ...
    if wait_for_acquisition_end(end_acquisition_img_proc, end_acquisition_img_proc_2, end_acquisition_img_proc_3):
        ...
        rate_c0 = total_c0 / elapsed_time if elapsed_time > 0 else 0

        print(f"Statistiques pour la position {position} mm:")
        print(f"\tTemps écoulé: {elapsed_time:.2f} secondes")
        print(f"\tThreshold: {dac_value}")
        print(f"\tC0: hits = {total_c0}, rate = {rate_c0:.2f} hits/s")
        print(f"Acquisition du fichier {file_name} terminée avec succès!")

        append_to_csv(
            csv_filename, position, delta, dac_value,
        ...
        print(f"Timeout détecté à la position {position} mm")
        stop_acquisition()
...
def main():
    ...
    try:
        ...
        # initialisation du système abeast
        ...
        # va à la première position une seule fois
        motor_move_relative(ser, start_position)
        print(f"Moteur déplacé à la position initiale : {start_position} mm")
        time.sleep(120)
        ...
        while current_position <= end_position:
            ...
            print(f"Threshold fixe = {FIXED_THRESHOLD}")
            print(f"{'=' * 50}\n")

            ok = perform_measurement_at_position(
            ...
            if current_position < end_position:
                next_step = min(step, end_position - current_position)
                print(f"Déplacement relatif supplémentaire de {next_step} mm")
                motor_move_relative(ser, next_step)

...
class NoiseScanEngine:
    ...
    def _run(
        ...
        try:
            ...
            total_scans = NUM_CHIPS * NUM_THRESHOLDS
            scan_index = 0
            if dac_start >= dac_end:
                dac_range = range(dac_start, dac_end - 1, -1)
            else:
                dac_range = range(dac_start, dac_end + 1)
            for chip in range(NUM_CHIPS):
                for th in range(NUM_THRESHOLDS):
                    ...
                    for dac in dac_range:
                        ...
                        xs.append(dac)
                        ys.append(rate)
                        self.log(f"DAC={dac} | counts={delta} | rate={rate:.2f} Hz")
                        ...
                        if zero_hits_streak >= 2:
                            self.log("Two consecutive DAC with 0 hits/s -> next threshold")
                            break
                        inflexion, slope, quality = self._fit_sigmoid_estimate(xs, ys)
                    ...
                    final_mu, final_slope, final_quality = self._fit_sigmoid_estimate(xs, ys)
                    ...
                    self.summary_rows.append({
                        ...
                        "mu_auto": final_mu,
                        "mu_auto": final_mu,
                        "sigma_auto": final_slope,
                        "r2_auto": final_quality,
    ...
    @staticmethod
    def _fit_sigmoid_estimate(xs: list[int], ys: list[float]):
        if len(xs) < 5:
            return None, None, None

        # tri par DAC (important si scan inversé)
        pairs = sorted(zip(xs, ys), key=lambda p: p[0])
        xs = [p[0] for p in pairs]
        ys = [p[1] for p in pairs]

        # lissage simple (optionnel mais important en pratique)
        ys_smooth = []
        window = 3

        for i in range(len(ys)):
            i0 = max(0, i - window)
            i1 = min(len(ys), i + window + 1)
            ys_smooth.append(sum(ys[i0:i1]) / (i1 - i0))
        ...
        # max pente → point d’inflexion
        idx = max(range(len(slopes)), key=lambda i: abs(slopes[i]))

        x0 = xs[idx]

        # pente locale = "raideur"
        k = slopes[idx]

        # qualité simple (optionnel)
        max_y = max(ys) if ys else 1
        min_y = min(ys) if ys else 0
        amplitude = max_y - min_y

        quality = abs(k) / amplitude if amplitude > 0 else 0

        x0_dac = int(round(x0))

        return x0_dac, k, quality
...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        ttk.Label(scan, text="DAC start").grid(row=5, column=0, sticky="w")
        self.dac_start_var = tk.StringVar(value="255")
        ttk.Entry(scan, textvariable=self.dac_start_var, width=10).grid(row=5, column=1)

        ttk.Label(scan, text="DAC end").grid(row=6, column=0, sticky="w")
        self.dac_end_var = tk.StringVar(value="0")
        ttk.Entry(scan, textvariable=self.dac_end_var, width=10).grid(row=6, column=1)
        ...
        cols = ("chip", "th", "dac", "hits", "counts", "mu", "sigma", "r2")
    ...
    def start_scan(self):
        try:
            ...
            if not self.fw.is_open():
                raise RuntimeError("Serial not connected")
            dac_start = int(self.dac_start_var.get())
            dac_end = int(self.dac_end_var.get())
    ...
    def refresh_chip_plot(self, chip: int):
        ...
        for th in range(NUM_THRESHOLDS):
            pts = self.curves_by_key.get((chip, th), [])
            xs = [p.dac for p in pts]
            ys = [p.hits_per_s for p in pts]
            # ===== SIGMOID FIT DISPLAY =====
            if len(xs) > 5:
                x0, k, q = self.engine._fit_sigmoid_estimate(xs, ys)
